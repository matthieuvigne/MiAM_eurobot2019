...
bool Robot::isLidarPointWithinTable(LidarPoint const& point)
{
  ...
  double const theta_T_R = this->isPlayingRightSide_
    ? robot_position.theta : 2*M_PI - robot_position.theta ;

  // 2. Project the Lidar point within the table
  double const T_x_fi = T_x_R + point.x * std::cos(theta_T_R);
  double const T_y_fi = T_y_R + point.y * std::sin(theta_T_R);

  // 3. Check if the lidar point falls within the table
  if(T_x_fi < table_dimensions::table_max_x)
  {
    if(T_y_fi < table_dimensions::ramp_min_y) return true;
    else if(T_y_fi < table_dimensions::table_max_y 
      and T_x_fi < table_dimensions::ramp_min_x
      and T_x_fi > table_dimensions::ramp_max_y)
        return true;
  }
...
double Robot::avoidOtherRobots()
{
  ...
  {
    num_stop_iters += 1;
    //~ std::cout << "[Robot.cpp l.393]: " << coeff << std::endl;
    return 0.;
  } 
  ...
  for(const DetectedRobot& robot : lidar_.detectedRobots_)
  {
    ...
    if(forward_) // If the robot is going forward
    {
      if(point.r < detection::r1 
        and ( point.theta < detection::theta1 
      ...
      else if(point.r < detection::r2)
      {
        ...
        {
          const double current_coeff = (point.r - detection::r1)
