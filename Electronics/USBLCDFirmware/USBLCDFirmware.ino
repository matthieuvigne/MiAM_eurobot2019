...
This file is a modification of Adafruit firmware from
https://github.com/adafruit/Adafruit-USB-Serial-RGB-Character-Backpack

that integrates the changes done by the addon board: the 4 GPIOs and RX/TX lines were
reconverted into three buttons and 3 LEDs.
Consequently:

 - the serial (UART) interface is disabled.
 - the commands MATRIX_GPO_OFF, MATRIX_GPO_ON, MATRIX_GPO_START_ONOFF, MATRIX_BAUDRATE now have not effect.
 - added command ADDON_GET_BUTTON_STATE 0xAA: return a single char containing 0b10000d2d1d0 where d1 is the state of
buttong i.
 - added command ADDON_SET_LED_STATE 0xAB: expert one parameter of the form 0b00000d2d1d0 where di is the state of
led i (1 for on, zero for off).

To compile, use arduino-1.0.5 with teensyduino 1.16. Get .hex from /tmp/ directory. Wire the board using ArduinoISP wiring and code.
Flash with:
avrdude -p usb162 -P /dev/ttyACM0 -c avrisp -b 19200 -U flash:w:USBLCDFirmware.cpp.hex -v
...
void setup() {
  ...
  // Set buttons as input, pullup, and leds as output.
    ...
    switch (c) {
      ...
      case MATRIX_SETCURSOR_POSITION:
       ...
       case MATRIX_SAVECUSTOMCHARBANK:
         ...
         readCustom(c, b);
         break;
        // Disable adafruit GPIO handling.
        /*
        case MATRIX_GPO_OFF:
        ...
        case MATRIX_GPO_START_ONOFF:
          ...
          break;
          */
       case ADDON_SET_LED_STATE:
         ledState = serialBlockingRead();
         for(uint8_t i = 0; i < 3; i++)
         {
           if((ledState & (1 << i)) > 0)
